...
/// `autopilot_run` AFTER that run's own `record_run` + completion signal
/// already fired, so a slow or failing LinkedIn fetch can never delay the
/// run's own "done" state or the "new jobs" notification. A per-URL failure
...
pub(crate) async fn enrich_linkedin_descriptions(app: AppHandle, urls: Vec<String>) {
    ...
    for url in &urls {
        ...
        let Some(description) = description.filter(|d| !description_is_blank(Some(d))) else {
            continue;
        };

        let req = crate::commands::scrape::ScrapeUpdateDescriptionRequest {
            url: url.clone(),
            description,
        };
        match crate::commands::scrape::scrape_update_description(app.clone(), req) {
            Ok(true) => enriched += 1,
            Ok(false) => {
